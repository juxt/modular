...
(defrecord SingleTemplate [template]
  RingBinding
...
(defn wrap-template
  ...
  [h]
  (fn [req]
    (if-let [merge-with-template (::template req)]
      (let [resp (h req)]
        (merge resp {:body (merge-with-template req resp)}))
      (throw (ex-info "wrap-template expects that a template is bound in the request with key :modular.template/template" {})))))
